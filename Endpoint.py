...
class AsyncServer(Server):
    ...
    async def handle_request(self, reader:asyncio.StreamReader, writer:asyncio.StreamWriter):
        try:
            ...
            try:
                ...
                # reciver sockets are supposed to get a graceful shut down and get sessionid removal
                if connection.is_receiver_socket:
                    logging.warning(f"Forcefully closing {connection} - proper cleanup failed")
            except ConnectionResetError:
                pass # connection already closed properly
            except Exception as e:
                # unexpected error
                traceback.print_exc()
    ...
    async def reverse_request(self, userid, view, data=None):
        try:
            ...
            traceback.print_exc()
            self.logger.error(f"Unexpected error while making a {view} reverse request to {userid} {str(e)}")
            return False
                ...
                # OPERATION 4 Read tree
                    ...
                    if pathlib.Path(operation_path).is_dir():
                        try:
                            ...
                            traceback.print_exc()
                            self.logger.error("UnexpectedError: Error while preparing read tree operation")
                    else:
